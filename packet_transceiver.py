...
def receive_UDP(ip, port):
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as server_sock:
        try:
            ...
            logger.info('receiving socket is in use!')
        while True:
...
def wait_for_remote_sync():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        sock.bind(('', 7999))
        sock.listen(1)
        logger.info('waiting for syncing with the remote side...')
        conn, addr = sock.accept()
        with conn:
            logger.info('got sync from ', addr)
...
def sync_remote(ip):
    ...
    while True:
        ...
        # if connected
        if result == 0:
            logger.info('synced connection with the remote side')
            break
        time.sleep(0.2)
...
def iterate_packet_per_device_py(estimatedDeviceParams, trafficExtremeValues, direction):
    ...
    for parameter in ['pktLen', 'IAT']:
        ...
        if isinstance(mix, sklearn.mixture.GaussianMixture):
            # logger.info(parameter, direction, mix.means_)
            ...
            while not ((genParam <= paramLimit['max']) and (genParam >= paramLimit['min'])):
                if parameter == 'pktLen':
                    ...
                    genParam = round(
                        np.asscalar(np.random.normal(mix.means_[component], np.sqrt(mix.covariances_[component]))))
                else:
                    genParam = np.asscalar(
                        np.random.normal(mix.means_[component], np.sqrt(mix.covariances_[component])))
...
def generate_packets(trafficParameters, args, direction, sock, remotePort=None):
    ...
    logger.info('Started generating packets...')
    ...
    while time.time() < generateUntil:
        for device in estimatedParameterEM:
            # logger.info(device)
            packetGenerator = iterate_packet_per_device_py(estimatedParameterEM[device], trafficExtremeValues[device],
                                                           direction)
    ...
    utils.save_obj(iat, direction + '_iat')
...
def get_first_datagram_port(sock):
    while True:
        ...
        logger.info('received first UDP from: ', addr)
        break
    return addr
...
def init_local_socket(args, LISTEN_PORT):
    if args.protocol == 'UDP':
        ...
        while True:
            ...
            # if connected
            if result == 0:
                logger.info('connected to the remote side')
                break
            time.sleep(0.1)
    else:
        exit('wrong transport protocol, see help')
...
def main():
    ...
    if args.i:
        ...
        logger.info('Saving pcap to: {}'.format(pcapToSave))
