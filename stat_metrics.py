...
def calc_stats(original, generated, prefix='', verbose=True):
    ...
    _, ps_series1 = scipy.signal.periodogram(original, scaling='spectrum')
    f, ps_series2 = scipy.signal.periodogram(generated, scaling='spectrum')
    # stats['series_acf_peak'] = window_estimator.freq_bounded_window_estimation(ps_series2, f,
    #                                                                            low_freq=LOW_FREQ,
    #                                                                            high_freq=HIGH_FREQ)
    stats['spectr_pearson'] = pearsonr(ps_series1, ps_series2)[0]
    # stats['spectr_product'] = np.sum(ps_series1 * ps_series2)
    stats['spectr_csd_sum'] = np.abs(np.sum(scipy.signal.csd(original, generated, scaling='spectrum')[1]))
    stats['spectr_dist'] = np.sum(np.abs(ps_series1 - ps_series2))
...
def calculate_stats_dfs(original, generated):
    new = utils.construct_dict_2_layers(original)
    for dev, direct, df in utils.iterate_2layer_dict(original):
        res = {}
        for feature in df.columns:
            res[feature] = calc_stats(df[feature], generated[dev][direct][feature], verbose=False)
        new[dev][direct] = pd.DataFrame(res)
    return new
