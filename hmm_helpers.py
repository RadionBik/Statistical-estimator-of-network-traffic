    ...
    for device, direction, df in iterate_2layer_dict_copy(traffic_dfs):
        ...
        for comp in comp_range:
            ...
            if bic[-1] < lowest_bic:
                ...
                best_comp = comp
        print('Best BIC is with {} components'.format(best_comp))
    return models
...
def gener_samples(models, scalers=None, sample_numb=500, truncate_negative=True, parameters=('IAT', 'pktLen')):
    ...
    for device, direction, df in iterate_2layer_dict(models):
        ...
        if truncate_negative:
            ...
            if scaler:
                samples[device][direction] = pd.DataFrame(scaler.inverse_transform(samples[device][direction]),
                                                          columns=parameters)
...
def generate_features_from_gmm_states(gmm_models, states_dict, scalers=None):
    ...
    for device, direction, gmm in iterate_2layer_dict(gmm_models):
        ...
        gen_packet = np.zeros(gmm.means_.shape[1])
        ...
        gener_df[device][direction] = pd.DataFrame(gen_samples, columns=['IAT', 'pktLen'])
